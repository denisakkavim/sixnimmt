...
class LLMBot(Bot):
    ...
    def act(self, view: MatchView, rejection: Rejection | None = None) -> Action:
        ...
        for attempt in range(self.options.repair_attempts + 1):
            ...
            response = self._request(messages, view, min(remaining, self.options.request_timeout_seconds))
            try:
    ...
    def _request(self, messages: list[dict[str, Any]], view: MatchView, timeout: float) -> Any:
