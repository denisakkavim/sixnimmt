"""Rules, player observations and action schemas shared by agent transports."""
...
def system_instructions(view: MatchView, rules_prompt: str, strategy_prompt: str) -> str:
    return match_instructions(view.protocol, view.target_score, rules_prompt, strategy_prompt)


def match_instructions(protocol: MatchProtocol, target_score: int, rules_prompt: str, strategy_prompt: str) -> str:
    ...
    if protocol.end_condition == "fixed_hands":
        ...
        settings = f"Match ends after a hand when anyone reaches {target_score} banked points; lowest score wins (ties share victory)."
    if protocol.communication_enabled:
